...
def gravar_produto(ficheiro_produto: str, produtos: List[Produto]) -> None:
    with open(ficheiro_produto, "w", encoding="utf-8") as file:  # Especificando o encoding correto
...
def carregar_produtos(ficheiro_produto: str) -> List[Produto]:
    try:
        with open(ficheiro_produto, "r", encoding="utf-8") as file:  # Especificando o encoding correto
            assert isinstance(file, TextIOWrapper)  # Garantir que o tipo seja o esperado
...
def gravar_cliente(ficheiro_cliente: str, clientes: List[Cliente]) -> None:
    with open(ficheiro_cliente, "w", encoding="utf-8") as file:  # Especificando o encoding correto
        assert isinstance(file, TextIOWrapper)  # Garantir que o tipo seja o esperado
...
def carregar_clientes(ficheiro_cliente: str) -> List[Cliente]:
    try:
        with open(ficheiro_cliente, "r", encoding="utf-8") as file:  # Especificando o encoding correto
            assert isinstance(file, TextIOWrapper)  # Garantir que o tipo seja o esperado
...
def gravar_compra(ficheiro_compras: str, compras: List[dict]) -> None:
    with open(ficheiro_compras, "w", encoding="utf-8") as file:  # Especificando o encoding correto
        assert isinstance(file, TextIOWrapper)  # Garantir que o tipo seja o esperado
        json.dump(compras, file, ensure_ascii=False)

def carregar_compras(ficheiro_compras: str) -> List[dict]:
    try:
        with open(ficheiro_compras, "r", encoding="utf-8") as file:  # Especificando o encoding correto
            assert isinstance(file, TextIOWrapper)  # Garantir que o tipo seja o esperado
